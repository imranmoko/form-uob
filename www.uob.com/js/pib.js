...
+function ($) {
  ...
  Carousel.prototype.slide = function (type, next) {
    ...
    if ($.support.transition && this.$element.hasClass('slide')) {
      ...
      $active
        ...
        if ($next.attr('data-bg') !== undefined) {
            var $parentbg = $next.attr('data-parentbg'),
                $nextbg = $($parentbg + ' .' + $next.attr('data-bg')),
                $activebg = $($parentbg + ' .active'),
                currRatio = ($(window).width() / $(window).height()).toFixed(2);

            if ($nextbg.attr('data-ratio') < currRatio) {
                $nextbg.css('background-size', '100% auto');
            } else {
                $nextbg.css('background-size', 'auto 100%');
            }

            $activebg.css('z-index', 0).removeClass('active');
            $nextbg.css('z-index', 1).removeClass('hidden');
            setTimeout(function(){
  ...
  var old = $.fn.carousel
    var bgUpdate = function($this){
        var currRatio = ($(window).width() / $(window).height()).toFixed(2);
        var $parentbg = $this.find('.item').attr('data-parentbg'),
            $activebg = $($parentbg + ' .bgdrop');
        if ($activebg.attr('data-ratio') < currRatio) {
            $activebg.css('background-size', '100% auto');
        } else {
            $activebg.css('background-size', 'auto 100%');
        }
    }
...
+function ($) {
  ...
  var Collapse = function (element, options) {
    ...
    this.transitioning = null
      this.height = (this.options.height) ? this.options.height : 'auto';
  ...
  Collapse.prototype.show = function () {
    ...
    var scrollSize = $.camelCase(['scroll', dimension].join('-'))
      ...
      if (this.height === 'auto') {
          ...
          $(this.options.parent + ' .accordion-toggle:not(.collapsed)').attr('data-toggle', 'collaps');
          $(this.options.parent + ' .accordion-toggle.collapsed').attr('data-toggle', 'collapse');
      }
  }
  ...
  $(document).on('click.bs.collapse.data-api', '[data-toggle=collapse]', function (e) {
    ...
    if (!data || !data.transitioning) {
      if ($parent) $parent.find('[data-toggle=collapse][data-parent="' + parent + '"]').not($this).addClass('collapsed')
        if ($parent) $parent.find('[data-toggle=collaps][data-parent="' + parent + '"]').not($this).addClass('collapsed')
    ...
    $(document).on('click.bs.collapse.data-api', '[data-toggle=collaps]', function (e) {
        e.preventDefault();
    });
...
//      modalReset(that.$element.find('.modal-dialog'));
...
(function ($) {
	...
	, americandate: function ( val, elem, self) {
	  ...
	  , americandate2: function ( val, elem, self) {
    	  var numeric = new RegExp("%5b0-9%5d.html\/");
    	  if(!numeric.test(val)){
    		 return false;
    	  }
        var parts = val.split(/[.\/-]+/);
	...
	, confirmnewpwd: function(val, elem2){
		if(val.toUpperCase()!=$(elem2).val().toUpperCase()){
			return false;
		}
		else{
			return true;
	...
	, checknewpwd: function(val, elem2){
		if(val ==$(elem2).val()){
			return false;
		}
		else{
	...
	, specialchar: function(val, nbWords){
		...
		function specialCharacter(val) {
			...
			var reLetterOrDigit = /[^0-9a-zA-Z_]/;
			return reLetterOrDigit.test(val);
		} 	 
	}
 , specialcharspace: function(val, nbWords){
		if (specialCharacter(val)){
			return false;
		}
		else {
		...
		function specialCharacter(val) {
			var reLetterOrDigit = /[^a-z\d\s]/i;
			return reLetterOrDigit.test(val);
		} 	 
	}
 , mobileno: function(val,nbWords ){
		...
		function mobile(val) {
			...
			var reLetterOrDigit = /^0[0-9]*/;
			return reLetterOrDigit.test(val);
		} 
		function _isLetter(val){
			var reLetter = /[a-zA-Z_]+/i;
			return reLetter.test(val);
		} 
	}
 
 
 , mobilenocheck: function(val,nbWords ){
		
		if(mobile(val)){

			return false;
		}else if(_isLetter(val)){
			return false;
		}else if(specialCharOtp(val)){
		...
		function specialCharOtp(val) {

			var reLetterOrDigit = /[^0-9]/;
			return reLetterOrDigit.test(val);
		} 
	}
 
 
	, characotp: function(val,nbWords ){

		if(specialCharOtp(val)){
	...
	, charcheck: function(val, nbWords ){

		if (!_isDigit(val))
		{ 
			return false;
		}
		else if(!_isLetterOrDigit(val)){
		...
		function _isDigit(val){
			var reDigit = /[0-9]+/i;
			return reDigit.test(val);
		}

		function _isLetterOrDigit(val){
			var reLetterOrDigit = /[a-zA-Z_]+/i;
			return reLetterOrDigit.test(val);
		}
	 ...
	 , emailcheck: function(val, nbWords ){
   	  var validated = false;
	   	 // alert(val);
   		if(!_isEmail(val)){
   		//	alert(validated);
   		...
   		function _isEmail(val){
  	    		 ...
  	    		 if (atpos <1 || dotpos<atpos+2 || dotpos+2>=val.length || val.trim().indexOf(" ") >= 0){
	 ...
	 , minvalue: function(val, minvalue ){
	   	  var validated = false;
		   	 // alert(val);
	   		if(!_minvalue(val,minvalue)){
	   		//	alert(validated);
	   			return validated; 
	   		}
	   		function _minvalue(val,minvalue){
	 ...
	 , minvalueequal: function(val, minvalue ){
	   	  var validated = false;
		   	 // alert(val);
	   		if(!_minvalue(val,minvalue)){
	   		//	alert(validated);
	...
	, charreview: function(val, nbWords){
	...
	, checkuserid: function(val, elem2){
		//alert ('CHECKUSERID IN PARSELY>>>>>>>>>>>>>>');
		//alert("check elem2:" + elem2 +"value:" + val);
		if(val.toUpperCase() == elem2){
			return false;
		}
		else{
			return true;
	...
	, specialchar2: function(val, nbWords){
        //alert ("inside specialchar2 method....");
		if (specialCharacter(val)){
			//alert("false case:" + val);
			return false;
		}
		else {
			//alert ("true case");
		...
		function specialCharacter(val) {
			//alert("char checking start>>>>>>>>>");
			var reLetterOrDigit =  /[^0-9a-zA-Z\_]/;
			return reLetterOrDigit.test(val);
		} 	 
	}

	
	, curandnewpwd: function(val, elem2){
		//alert("inside curandnewpwd method in parsely...");
		
		
		//alert("elem2 original:" + $(elem2).val());
		
		//alert ("val:" + val);
		elem2=$(elem2).val();
		
		
		/*alert("elem2 to uppercase:" + elem2);
	...
	, chkswitchunits: function(val, nbWords ){
		...
		function isPositiveDecimalValue(val){
			//var regex = "^(?=\d*(\.\d{1,4})?$)(?=.*[1-9])";
								...
								, afterdate:      "This date should be after %s."
									, luhn:           "This value should pass the luhn test."
										, americandate:	"This value should be a valid date (MM/DD/YYYY)."
											 , americandate2:	"This value should be a valid date (DD/MM/YYYY)."
												, confirmnewpwd:  "Confirm Password is different from New Password."
													, charcheck: 		"This value should have at least one character and one number."
														, checknewpwd:    "New Password should be different from User Name."
															, specialchar:  	"This value should be a valid value."
																, characotp: 		"This value should be a valid number."
																	, charreview: 	"The password cannot contain letters only."
																		,checkuserid: "New Password should be different from User Name."
																			,specialchar2:"This value should be a valid value."
																				, curandnewpwd : "Current Password and New Password should not be the same."
												,emailcheck: "Please input valid email address."
													,minvalue:	"Please input valid amount."
														, minvalueequal: "This value is required."
														,mobileno: "Please input valid mobile number."
														,chkswitchunits:"Please input valid number."
															,mobilenocheck: "Please input valid mobile number."
...
$(function(){
	...
	$('.easypiechart').each(function(){
		...
		$data.onStop =  function(value){
			$step.parent().html('<span class="h2 step" style="color: ' + $data.barColor + '">' + (parseInt(value) >= parseInt($data.pass) ? 'Pass' : 'Fail') + '</span>');
	...
	$(".datepicker-input").each(function(){
  	...
  	$(this).before('<div class="datepicker-calendar"><img src="images/icon-datepicker.html"></div>');
	});
	
	/*$('body').on('click', '.datepicker-calendar img', function(){
		$(this).blur();
		$(this).closest('div.form-group').find('input').b();
		setTimeout(function(){
	...
	*/
	...
	if($("html").hasClass("ios")){
	    $('body').on('click', '.calendar-input-selector', function(){
	    	$(this).blur();
	    	setTimeout(function(){
	    		$(this).focus();
	    	}, 10);
...
var defaults = {
 ...
 $window = $('section.scrollable.wrapper'),
 ...
 scroller = function() {
   ...
   for (var i = 0; i < sticked.length; i++) {
     ...
     var s = sticked[i],
         extras = (i > 0) ? 20 : 0;
       elementTop = s.stickyWrapper.offset().top + scrollTop,
       etse = elementTop - s.topSpacing  + extras;
//     console.log(scrollTop, etse, elementTop, s.topSpacing, $window.outerHeight(), $window.height(), ($window.outerHeight() - $window.height())/2, ($document.outerHeight() - $document.height())/2, Math.floor(($window.outerHeight() - $window.height())/2) - Math.floor(($document.outerHeight() - $document.height())/2));
     if (scrollTop <= etse) {
...
//           .css('width', '')
       ...
       if (s.currentTop != newTop) {
         s.stickyElement
           .css('position', 'fixed')
           .css('top', newTop)
//             .css('width', s.stickyElement.parent().width());
//           .css('z-index', s.zindex);
         //console.log(s.stickyElement.parent().width())
 ...
 resizer = function() {
   windowHeight = $window.outerHeight();
     windowWidth = $window.width();
     var tSpacing = $('.navbar').outerHeight();
     for (var i = 0; i < sticked.length; i++) {
         var s = sticked[i];
         s.topSpacing = tSpacing;
         tSpacing = tSpacing + s.stickyElement.outerHeight();
   ...
   init: function(options) {
     ...
     return this.each(function() {
       ...
       $(window).on('resize', function(){
         ...
         $(document).on('click', '.modal-dialog .btn-primary, .accordion-toggle, #btn-sidebar, [href="#nav"], input[type=checkbox]', function(){
             var timer = 1;
             if ($(this).hasClass('accordion-toggle') || $(this).hasClass('btn-primary')) {
                 timer = 300;
             }
             setTimeout(resizer, timer);

             if ($(this).hasClass('btn-primary')) {
                 setTimeout(resizer, timer + 5500);
             }
         });
...
//bootstrap-datepicker.sticky.js
/* =========================================================
